// Thin harness wrapper: exposes the IntrHalt field0 storage (via
// mkGoldIntrHalt) under the gate side's common port list = a hw-write
// input, a sw masked-merge (woclr) write input, and the stored value
// read (rd). See GoldIntrHalt.bsv for the expected-failing (intr OR-merge)
// disclosure.
module gold_top (
    CLK,
    RST_N,
