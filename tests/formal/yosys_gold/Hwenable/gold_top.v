// New thin harness wrapper (touches neither side's logic): ties
// EN_hw_clear=0 so the unmodeled generic hw.clear() boilerplate is
// never exercised on either side of the SAT equivalence check.
module gold_top (
    CLK,
    RST_N,
    hw__write_data,
  ...
  input [7:0] set_ext_top_gate_sig_v;
  ...
  gold_inner u (
      ...
      .set_ext_topmap_gate_sig_v(set_ext_top_gate_sig_v),
      .EN_set_ext_topmap_gate_sig(EN_set_ext_top_gate_sig),
      .RDY_set_ext_topmap_gate_sig(RDY_set_ext_top_gate_sig)
  );
endmodule
